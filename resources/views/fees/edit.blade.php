                ...
                <div class="p-5">
                    ...
                    <form action="{{ route('dashboard.wallet.fee.update',$fee['id']) }}" method="POST"
                                ...
                                <div class="sm:w-5/6 tillselect-marging">
                                    ...
                                    <select name="base_currency" id="base_currency" class="tom-select form-control"
                                        ...
                                        @foreach ($ledgers as $ledger)
                                            <option value="{{ $ledger->getKey() }}" @if(old('base_currency', $fee['base_currency']) == $ledger->getKey()) selected @endif>{{ $ledger->name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-span-12 md:col-span-8 xl:col-span-6 form-inline mt-2">
                                ...
                                <div class="sm:w-5/6 tillselect-marging">
                                    <select name="exchange_currency" id="exchange_currency" class="tom-select form-control"
                                        data-search="true">
                                        @foreach ($ledgers as $ledger)
                                            <option value="{{ $ledger->getKey() }}" @if(old('exchange_currency', $fee['exchange_currency']) == $ledger->getKey()) selected @endif>{{ $ledger->name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-span-12 lg:col-span-12 xl:col-span-6 form-inline mt-0">
                                ...
                                <div class="sm:w-5/6 tillselect-marging">
                                    <select class="tom-select form-control" data-search="true" name="payment_type">
                                        <option value="deposit" @if(old('payment_type', $fee['payment_type']) == 'deposit') selected @endif>Deposit</option>
                                        <option value="payout" @if(old('payment_type', $fee['payment_type']) == 'payout') selected @endif>Payout</option>
                                        <option value="withdraw" @if(old('payment_type', $fee['payment_type']) == 'withdraw') selected @endif>Withdraw</option>
                                    </select>
                        ...
                        <div class="grid grid-cols-12 md:gap-0 lg:gap-3 xl:gap-10 mt-2" @if (old('fee_type',$fee['fee_type']) == 'amount') x-data="{ selected: '1' }" @elseif (old('fee_type',$fee['fee_type']) == 'percentage') x-data="{ selected: '0' }" @else x-data="{ selected: '3' }" @endif>
                            <div class="items-center justify-center col-span-12 lg:col-span-12 xl:col-span-6 form-inline mt-0">
