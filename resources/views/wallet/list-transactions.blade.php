<div class="intro-y mt-3">
    <div class="sm:flex items-center sm:py-0 border-b border-gray-200 dark:border-dark-5">
        ...
        @if (isset($transactionType) && \Illuminate\Support\Facades\Auth::user()->isSubscriber())
            ...
            <a href="{{ route('dashboard.wallet.deposit.create',['workspace_id' => $workspace->id]) }}" class="btn btn-sm btn-primary shadow-md sm:ml-2">Deposit</a>
            @elseif ($transactionType == 'payout')
            <a href="{{ route('dashboard.wallet.payout.create',['workspace_id' => $workspace->id]) }}" class="btn btn-sm btn-primary shadow-md sm:ml-2">Payout</a>
            @endif
        @endif
    </div>
</div>
