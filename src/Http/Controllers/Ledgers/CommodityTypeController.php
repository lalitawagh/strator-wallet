...
class CommodityTypeController extends Controller
{
    ...
    public function store(StoreCommodityRequest $request)
    {
        ...
        $isExist = collect(Setting::getValue('commodity_types', [])->where('name',$data['name']));

        if(!empty($isExist) && count($isExist)>0){
            return redirect()->route("dashboard.wallet.commodity-type.create")->with([
                'status' => 'error',
                'message' => 'Commodity name is already exist.',
            ]);
        }
